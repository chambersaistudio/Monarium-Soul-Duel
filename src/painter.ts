...
import { FlarepawAnimation, FlarepawSheet } from './flarepawSheet';
...
export class Painter {
  private readonly frameDebug = new URLSearchParams(location.search).has('frames');

  constructor(private c: CanvasRenderingContext2D, private flarepawSheet: FlarepawSheet) {
  ...
  drawDuel(s: GameState) {
    ...
    this.drawHud(s); if (this.frameDebug) this.drawFrameDebug();
  }
  ...
  flarepaw(c: CanvasRenderingContext2D, f: Fighter, time: number) {
    const animation: FlarepawAnimation = f.attack > 0 || f.guard ? 'action' : !f.grounded ? 'jump' : Math.abs(f.vx) > 55 ? 'run' : 'idle';
    if (this.flarepawSheet.draw(c, animation, time)) {
    ...
    // Non-breaking fallback while the transparent sheet is unavailable during local setup.
  ...
  drawFrameDebug() {
    const c = this.c, ready = this.flarepawSheet.ready;
    this.pill(22, 124, 260, 54, '#09071ddd', ready ? '#65e6a5' : '#ff7d7d');
    this.text(ready ? 'FLAREPAW SHEET READY' : 'FLAREPAW SHEET FALLBACK', 38, 147, 11, ready ? '#8fffc4' : '#ffaaaa');
    this.text(ready ? `${this.flarepawSheet.image.naturalWidth}×${this.flarepawSheet.image.naturalHeight}  ·  FRAME ${this.flarepawSheet.frameWidth}×${this.flarepawSheet.frameHeight}` : 'Expected transparent sheet at configured path', 38, 165, 9, '#c7b9e8');
  }
