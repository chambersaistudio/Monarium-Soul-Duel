...
export class StoryOverworldScene extends Phaser.Scene {
  ...
  private startRivalBattle(): void { if (!this.state.starter || !this.state.renzoStarter) { this.ui.showDialogue([{ speaker: 'Renzo', portrait: this.portrait('renzo'), text: 'Get a partner from the Bond Lab first.' }]); return; } const ctx: ClassicBattleContext = { returnMap: 'training_field', returnSpawn: 'south', playerMinariId: this.state.starter, enemyMinariId: this.state.renzoStarter, bondable: false, battleType: 'rival', playerLevel: 7, enemyLevel: 8 }; this.registry.set('classic_battle_context', ctx); applyHighDpiCanvas(this.game, 'story:rival-battle:sync-now'); requestAnimationFrame(() => { applyHighDpiCanvas(this.game, 'story:rival-battle:sync-raf'); window.setTimeout(() => { applyHighDpiCanvas(this.game, 'story:rival-battle:sync-settled'); this.scene.start('ClassicSoulDuelScene'); }, 80); }); }
