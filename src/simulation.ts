...
function updateAI(state: GameState, dt: number) {
  ...
  if (ai.cooldown <= 0) {
    ...
    if (distance > 260 && roll < .75) {
      beginAttack(state, ai, 'splash', .6); ai.cooldown = 1.2;
    ...
    } else { ai.vx += ai.facing * 700 * dt; ai.cooldown = .25; }
  }
  ai.guard = p.attack > 0 && distance < 170 && Math.random() < .04;
}

function updateDuel(state: GameState, input: Input, dt: number) {
  updatePlayer(state, input, dt); updateAI(state, dt);
  for (const f of state.fighters) {
    f.cooldown -= dt; f.stagger -= dt; f.dodge -= dt; f.attack -= dt; f.comboWindow -= dt; f.form -= dt;
    f.aura = clamp(f.aura + dt * 4, 0, 100); f.vy += 1450 * dt; f.x += f.vx * dt; f.y += f.vy * dt;
